...
const auth = new JWT({
  ...
  // KODE DI BAWAH INI ADALAH VERSI PALING AMAN:
...
app.get('/api/daftar-nama', async (req, res) => {
  ...
  try {
    ...
    await sheet.loadCells({
        startRowIndex: startRow - 1, endRowIndex: startRow + 30,
        startColumnIndex: 1, endColumnIndex: 2
    });

    let daftarNama = [];
    for (let i = 0; i < 31; i++) {
      let val = sheet.getCell(startRow - 1 + i, 1).value;
...
app.post('/api/simpan', async (req, res) => {
  ...
  try {
    ...
    let rowIdx = -1;
    for (let i = 0; i < 31; i++) {
      if (sheet.getCell(startRow - 1 + i, 1).value?.toString().trim() === data.nama.trim()) {
          rowIdx = startRow - 1 + i; break;
      }
    }

    if (rowIdx === -1) return res.status(404).json({ status: "Error", msg: "Nama tidak ditemukan" });
    ...
    const prompt = "Berikan 1 kalimat motivasi islami singkat untuk mahasiswa. Tanpa kutip.";
    const result = await model.generateContent(prompt);
    
    res.json({ status: "Sukses", motivasi: result.response.text() });
  } catch (e) {
    res.status(500).json({ status: "Error", msg: e.message });
  }
});

// 4. Analisis & Grafik (Opsional untuk pengembangan)
app.get('/api/grafik-data', async (req, res) => {
    // Logika pengambilan data lama untuk grafik bisa ditaruh di sini
    res.json([]); 
});

const PORT = process.env.PORT || 3000;
// Hapus app.listen, ganti dengan ini:
module.exports = app;